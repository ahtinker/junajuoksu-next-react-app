...
export default function PrivacyPolicyPage() {
    ...
    React.useEffect(() => {
        const fetchMarkdown = async () => {
            try {
                const res = await fetch(`/legal/privacy/${locale}.md`);
                if (res.ok) {
    ...
    return (
        <div className="App" style={{ backgroundColor: 'var(--bulma-scheme-main)' }}>
            <NavBar />
            <section className="container has-text-left my-6 px-4" style={{ backgroundColor: 'var(--bulma-scheme-main)', minHeight: '100vh' }}>
                <Markdown
